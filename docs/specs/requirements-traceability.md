...
| `AW-REQ-002` | submit時に必須項目妥当性を満たす | `src/domain/engine.ts` (`createRequest`, `updateRequest` の入力検証) | 正常系テストはあり（受入/API） | 部分達成 | 必須項目欠落の直接負例テストは未追加 |
| `AW-REQ-003` | submit時にworkflow/versionを固定 | `src/domain/engine.ts` (`workflowId`, `workflowVersion` 固定) | `tests/acceptance/approval-engine.acceptance.test.ts` (`AW-WF-002`) | 要確認 | `RETURNED -> resubmit` で再選択される実装。要求解釈との整合確認が必要 |
...
| `AW-WF-003` | 申請中はworkflow固定（定義変更非影響） | `src/domain/engine.ts` (`request.workflowId/version` 保持) | 直接テストなし | 要確認 | 定義更新APIがないため通常経路は維持。resubmit時の再選択挙動は別途判断必要 |
...
| `AW-AUD-001` | 提出〜決裁の監査ログを保持 | `src/domain/engine.ts` (`addAudit`呼出し群) | `tests/acceptance/approval-engine.acceptance.test.ts` (`AW-ACC-04`), `tests/integration/api.acceptance.test.ts` (`AW-ACC-04`) | 部分達成 | 提出〜承認系は検証済み。withdraw/cancel全件網羅は未追加 |
...
| 区分 | 内容 | 推奨対応 |
| --- | --- | --- |
| 要確認 | `AW-WF-003`（workflow固定）と `RETURNED -> resubmit` の再選択挙動が要求解釈と一致するか | 仕様判断をIssue #1で明示し、必要なら`submitRequest`の固定ロジックを修正 |
| テスト不足 | `AW-REQ-002` の必須項目欠落時の直接負例テスト | 受入/APIに `title/amount/currency` 欠落パターンを追加 |
| テスト不足 | `AW-AUD-001` の withdraw/cancel を含む監査ログ網羅確認 | 受入/APIに `REQUEST_WITHDRAW` / `REQUEST_CANCEL` のログ検証を追加 |
